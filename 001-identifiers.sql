...
create function meta.field_id(schema_name text,relation_name text,pk_column_name text,pk_value text,column_name text) returns meta.field_id as $_$ select row(schema_name,relation_name,pk_column_name,pk_value,column_name)::meta.field_id $_$ immutable language sql;
...
create type meta.row_id as (schema_name text,relation_name text,pk_columns text[],pk_values text[]);
create function meta.row_id(schema_name text,relation_name text,pk_columns text[],pk_values text[]) returns meta.row_id as $_$ select row(schema_name,relation_name,pk_columns,pk_values)::meta.row_id $_$ immutable language sql;
...
create function meta.eq(leftarg meta.row_id, rightarg jsonb) returns boolean as $_$select ((leftarg).schema_name)::text = (rightarg)->>'schema_name' and ((leftarg).relation_name)::text = (rightarg)->>'relation_name' and to_jsonb((leftarg).pk_columns) = rightarg->'pk_columns' and to_jsonb((leftarg).pk_values) = rightarg->'pk_values'$_$ immutable language sql;
create operator meta.= (leftarg = meta.row_id, rightarg = jsonb, procedure = meta.eq);
create function meta.row_id(value jsonb) returns meta.row_id as $_$select meta.row_id((value->>'schema_name')::text, (value->>'relation_name')::text, (select array_agg(value) from jsonb_array_elements_text(value->'pk_columns')), (select array_agg(value) from jsonb_array_elements_text(value->'pk_values'))) $_$ immutable language sql;
create cast (jsonb as meta.row_id) with function meta.row_id(jsonb) as assignment;
create function meta.eq(leftarg meta.row_id, rightarg json) returns boolean as $_$select ((leftarg).schema_name)::text = ((rightarg)->>'schema_name')::text and ((leftarg).relation_name)::text = ((rightarg)->>'relation_name')::text and to_json((leftarg).pk_columns)::text = (rightarg->'pk_columns')::text and to_json((leftarg).pk_values)::text = (rightarg->'pk_values')::text$_$ immutable language sql;
create operator meta.= (leftarg = meta.row_id, rightarg = json, procedure = meta.eq);
create function meta.row_id(value json) returns meta.row_id as $_$select meta.row_id((value->>'schema_name')::text, (value->>'relation_name')::text, (select array_agg(value) from json_array_elements_text(value->'pk_columns')), (select array_agg(value) from json_array_elements_text(value->'pk_values'))) $_$ immutable language sql;
